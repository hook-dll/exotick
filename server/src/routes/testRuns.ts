...
const ITEM_COLUMNS =
  'id, test_run_id, test_case_id, snapshot_description, snapshot_notes, snapshot_section_name, snapshot_sub_module_name, snapshot_module_name, order_index, status, updated_by';
    ...
    const run = transaction(() => {
      ...
      const cases = db.prepare(
        `SELECT tc.id, tc.description, tc.notes, tc.library_id,
                s.name AS section_name, sm.name AS sub_module_name, m.name AS module_name
         FROM test_cases tc
         LEFT JOIN sections s ON s.id = tc.section_id
         LEFT JOIN sub_modules sm ON sm.id = tc.sub_module_id
         LEFT JOIN modules m ON m.id = tc.module_id
         WHERE tc.id IN (${placeholders})
         ORDER BY
           CASE WHEN tc.module_id IS NULL THEN 1 ELSE 0 END,
           m.order_index, m.id,
           CASE WHEN tc.sub_module_id IS NULL THEN 1 ELSE 0 END,
           sm.order_index, sm.id,
      ...
      const insertItem = db.prepare(
        `INSERT INTO test_run_items
         (test_run_id, test_case_id, snapshot_description, snapshot_notes, snapshot_section_name, snapshot_sub_module_name, snapshot_module_name, order_index)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      );
      let orderIndex = 0;
      for (const tc of cases) {
        insertItem.run(runId, tc.id, tc.description, tc.notes ?? null, tc.section_name ?? null, tc.sub_module_name ?? null, tc.module_name ?? null, orderIndex++);
      }
